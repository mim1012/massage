    ...
    <div className="max-w-[1400px] mx-auto px-3 py-3">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-4 flex items-center justify-between rounded-lg bg-gradient-to-r from-red-600 to-rose-500 p-4 text-white">
            <div>
          ...
          <div className="mb-3 flex gap-1.5 overflow-x-auto pb-2 md:hidden scrollbar-hide">
            <Link
              href="/"
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {premiumShops.map((shop) => (
                  <Link
                    key={shop.id}
                    href={`/shop/${shop.slug}`}
                    className="banner-item flex overflow-hidden rounded-2xl border-2 border-amber-300 bg-white transition-all hover:-translate-y-1 hover:shadow-xl"
                  >
                    ...
                    <div className="flex min-w-0 flex-1 flex-col justify-center p-3 sm:p-4">
                      <div className="mb-2 flex items-start justify-between gap-1">
                        <div className="min-w-0">
                          <div className="mb-1 flex items-center gap-1.5">
                            <span className="rounded bg-amber-500 px-1.5 py-0.5 text-[10px] font-black text-white sm:text-xs">
                              AD
                            </span>
                        ...
                        <div className="flex shrink-0 items-center gap-1 rounded-lg bg-amber-50 px-2 py-1">
                          ...
                          <span className="text-sm font-bold text-amber-700">{shop.rating.toFixed(1)}</span>
                        </div>
                      </div>
                      <div className="mb-2 flex items-center gap-2 text-xs text-gray-500 sm:text-sm">
                        ...
                        <span className="font-medium text-red-500">#{shop.themeLabel}</span>
                      </div>
            ...
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-400">
                  ({regularShops.length}개)
                </span>
              </div>
              <div className="flex items-center gap-2">
                {sortType === 'popular' && (
                  <Link href="/" className="text-[11px] font-bold text-red-600 hover:underline">
                    정렬 초기화
                  </Link>
                )}
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="flex items-center gap-1 rounded border border-gray-200 bg-gray-50 px-2 py-1 text-xs text-gray-500 transition-colors hover:bg-red-50 hover:text-red-600 disabled:opacity-50"
                >
          ...
          <div className="seo-content mt-6 rounded-lg border border-gray-200 bg-white p-5">
            <h1 className="mb-3 text-xl font-bold">{homeSeo.section1Title}</h1>
            <p className="mb-6 text-sm leading-relaxed text-gray-600">{homeSeo.section1Content}</p>

            <h2 className="mb-2 text-lg font-bold">{homeSeo.section2Title}</h2>
            <p className="mb-6 text-sm leading-relaxed text-gray-600">{homeSeo.section2Content}</p>

            <h2 className="mb-2 text-lg font-bold">{homeSeo.section3Title}</h2>
        ...
        <aside className="hidden w-[120px] shrink-0 lg:block">
          <div className="sticky top-[170px] relative z-10 space-y-2">
            ...
            <div className="flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white text-center shadow-sm divide-y divide-gray-100">
              <div className="bg-gray-100 py-1.5 text-[11px] font-bold text-gray-700">QUICK MENU</div>
              <Link
                href="/?view=list"
                className="group flex flex-col items-center gap-1 py-2 transition-colors hover:bg-red-50 hover:text-red-600"
              >
