...
type Props = {
  initialInquiryDoc: PartnershipInquiryDoc;
};
  ...
  if (submitted) {
    return (
      <div className="mx-auto max-w-[600px] px-4 py-12 text-center sm:py-20">
        ...
        <p className="mb-10 whitespace-pre-line leading-relaxed text-gray-600">{inquiryDoc.note}</p>
        <Link
          href="/"
  ...
  return (
    <div className="mx-auto max-w-[800px] px-3 py-6">
      <div className="mb-4 flex items-center gap-1 text-xs text-gray-500">
        ...
        <span className="font-bold text-gray-800">입점 문의</span>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xl sm:rounded-3xl">
        <div className="bg-gradient-to-br from-[var(--portal-brand-dark)] via-[var(--portal-brand-hover)] to-[var(--portal-brand)] p-6 text-center text-white sm:p-10">
          <h1 className="mb-2 text-xl font-black uppercase italic tracking-wider sm:mb-4 sm:text-4xl">{inquiryDoc.eyebrow}</h1>
          <div className="mx-auto mb-4 h-1 w-12 rounded-full bg-white/30" />
          <p className="mb-1 text-sm font-bold opacity-90 sm:text-2xl">{inquiryDoc.title}</p>
          <p className="whitespace-pre-line text-xs font-medium leading-relaxed opacity-95 sm:text-base">{inquiryDoc.description}</p>
        </div>
