...
export default function ShopEditorPage({ params, routeBase }: Props) {
  ...
  const inputClassName = 'w-full rounded border border-gray-300 px-3 py-1.5 text-sm focus:border-red-500 focus:outline-none';
    ...
    <div className="max-w-[800px] space-y-4 pb-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="rounded border border-gray-200 bg-white p-4">
          ...
          <div className="mb-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="text" required value={form.slug} onChange={(event) => setForm({ ...form, slug: event.target.value })} className={inputClassName} />
            </div>
          </div>
          ...
          <div className="mb-3">
            ...
            <input type="text" value={form.tagline} onChange={(event) => setForm({ ...form, tagline: event.target.value })} className={inputClassName} />
          </div>

          <div className="mb-3">
            <label className={labelClassName}>설명</label>
            <textarea rows={4} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} className={`${inputClassName} resize-none`} />
          </div>

          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              <label className={labelClassName}>연락처</label>
              <input type="text" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} className={inputClassName} />
            </div>
            <div>
              <label className={labelClassName}>운영 시간</label>
              <input type="text" value={form.hours} onChange={(event) => setForm({ ...form, hours: event.target.value })} className={inputClassName} />
            </div>
          </div>

          <div className="mt-3">
            <label className={labelClassName}>주소</label>
            <input type="text" value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} className={inputClassName} />
          </div>

          <div className="mt-3">
            <label className={labelClassName}>태그</label>
            <input type="text" value={tagsStr} onChange={(event) => setTagsStr(event.target.value)} placeholder="예: 스파, 주차가능, 심야영업" className={inputClassName} />
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {courses.map((course, index) => (
              <div key={`${course.name}-${index}`} className="flex items-start gap-2">
                <input type="text" placeholder="코스명" value={course.name} onChange={(event) => updateCourse(index, 'name', event.target.value)} className={`${inputClassName} w-1/3`} />
                <input type="text" placeholder="소요 시간" value={course.duration} onChange={(event) => updateCourse(index, 'duration', event.target.value)} className={`${inputClassName} w-1/4`} />
                <input type="text" placeholder="가격" value={course.price} onChange={(event) => updateCourse(index, 'price', event.target.value)} className={`${inputClassName} flex-1`} />
                <button type="button" onClick={() => setCourses((current) => current.filter((_, courseIndex) => courseIndex !== index))} className="rounded p-2 text-red-500 hover:bg-red-50">
        ...
        <div className="flex justify-end gap-2">
          <Link href={routeBase} className="rounded border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">
            취소
          </Link>
          <button type="submit" className="flex items-center gap-1 rounded bg-red-600 px-4 py-2 text-sm font-bold text-white hover:bg-red-700">
